...
- `SubscriberCollection` instances can live for a long time inside events and recyclables. When subscription churn drops well below a list's peak capacity, trim the backing lists instead of retaining the largest historical size forever.
- `SubscriberCollection.Subscribe(...)` intentionally rejects already-invalid subscriptions and folds older pending additions into the main list before adding more when it is not inside a notification pass. This keeps sparse events from retaining dead pending subscriptions indefinitely while avoiding list mutation during callbacks.
- `SubscriberCollection.RemoveEntryAt(...)` must dispose the removed subscription with its captured lease before recycling the lightweight subscriber entry. Removing without disposal leaks the pooled subscription object and forces future `ScopedSubscription<T>` / `ArgsSubscription<T>` rents to allocate new instances.
- `SubscriberCollection` has a single-subscriber fast path. Do not reintroduce eager `List<T>` fields or a heap-allocated subscriber entry type; most disposal collections only carry one callback, and list/object allocations there show up directly in gameplay profiles.
- `Recyclable.OnDisposed(...)` keeps the first disposal subscriber inline and creates a `SubscriberCollection` only when a second disposal callback is registered. Preserve this because most pooled lifetimes have exactly one disposal callback.
