...
public static class SoftPad
{
    ...
    public static ISynthPatch Create(NoteExpression note)
    {
        ...
        var attack = Lerp(1f, 2f, vNorm);                                         // 3ms -> 1.5ms
        var decay = (float)Math.Clamp(LerpD(0.25, 1, dur01), 0.03, 0.18);             // ~short -> longer
        var sustain = Lerp(0.88f, 0.96f, vNorm);                                           // more push on high velocity
        var release = (float)Math.Clamp(LerpD(1, 2, dur01), 0.08, 0.35);             // stabs don’t smear; long notes breathe
        ...
        var driftDepth = Lerp(0.45f, 0.95f, dur01);
        var driftRate = Lerp(7.5f, 3.5f, dur01);
        ...
        var revDur = Lerp(0.18f, 0.35f, dur01);
        ...
        return LayeredPatch.CreateBuilder()
            ...
            (
                ...
                patch: SynthPatch.Create(note)
                    ...
                    .WithVolume(patchVol)
                    .WithPresenceShelf(new PresenceSettings { GainDb = 1.7f.Param().WithClamp(-24f, +24f).WithSmoothing(0.45f).Route(Env.Amp(), depth: +16.5f) })
