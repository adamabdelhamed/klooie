namespace klooie.Gaming;
    ...
    public class Wander : Movement
    {
            ...
            try
            {
                while (this.IsExpired == false)
                {
                    ...
                    if (_CuriosityPoint != null && cpd <= Options.CloseEnough)
                    {
                        ...
                        var cpdPrime = cpd * 1.01f;
                        var newSpot = Element.OffsetByAngleAndDistance(a, cpdPrime);
                        Element.MoveTo(newSpot.Left, newSpot.Top);
                        lkg = a;
                    ...
                    else
                    {
                        ...
                        Velocity.Speed = LastStuckTime.HasValue && Game.Current.MainColliderGroup.Now - LastStuckTime.Value < TimeSpan.FromSeconds(5) ? 2f : Speed();
        ...
        private async Task YieldForVelocityAndDelay()
        {
            ...
            await EarliestOf(this, Velocity.OnVelocityEnforced.CreateNextFireLifetime()).AsTask();
            await AssertAlive();
        }
