...
public abstract class Weapon : ObservableObject, IInventoryItem
{
    ...
    public virtual float Strength { get => Get<float>(); set => Set(value); }
    public ConsoleString DisplayName { get; set; }

    public SmartTrigger Trigger { get; set; } = new SmartTrigger();
    ...
    public void TryFire(bool alt)
    {
        ...
        {
            Game.Current.Sound.Play("debit");
            OnFireEmpty.Fire(this);
            return;
        }

        if (Trigger.AllowFire() == false) return;
