...
public class Inventory : ObservableObject
{
    ...
    public Weapon PrimaryWeapon
    {
        ...
        set
        {
            if (value == null && PrimaryWeapon == null) return;
            if (value != null && value == PrimaryWeapon) return;
    ...
    public Weapon ExplosiveWeapon
    {
        ...
        set
        {
            if (value == null && ExplosiveWeapon == null) return;
            if (value != null && value == ExplosiveWeapon) return;
            if (value != null && Items.Contains(value) == false)
            {
                Items.Add(value);
            }
    ...
    public Inventory()
    {
        this.Subscribe(nameof(Items), () =>
        {
            itemsLifetime?.Dispose();
            itemsLifetime = new Lifetime();
            Items.ForEach(item => ProcessItem(item));
            Items.Added.Subscribe((item) => ProcessItem(item), itemsLifetime);

        }, this);

        this.Subscribe(nameof(Owner), () =>
        {
    ...
    private void ProcessItem(IInventoryItem item)
    {
        ...
        if (item is Weapon)
        {
            ...
            {
                if (PrimaryWeapon == null || PrimaryWeapon.AmmoAmount == 0 || highestPrimaryWeapon == null || weapon.Strength > highestPrimaryWeapon.Strength)
                {
            ...
            {
                if (ExplosiveWeapon == null || ExplosiveWeapon.AmmoAmount == 0 || highestExplosiveWeapon == null || weapon.Strength > highestExplosiveWeapon.Strength)
                {
