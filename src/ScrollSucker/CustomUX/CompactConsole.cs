...
public abstract class CompactConsole : ConsolePanel
{
    ...
    public void HardRefresh(ConsoleString outputValue = null)
    {
        ...
        if (myLt == refreshLt)
        {
            ...
            if (InputBox.CanFocus && InputBox.IsFocusStackAtMyLevel)
            {
                InputBox.Focus();
            }
        }
    ...
    private async Task OnHandleHey(ConsoleKeyInfo keyInfo)
    {
        ...
        if (keyInfo.Key == ConsoleKey.Enter)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (def.Metadata.HasMeta<SuppressBackspaceMessageAttribute>() == false)
                {
                    output += ConsoleString.Parse("\n\n[White]press [B=Cyan][Black] backspace [D][White] to list available commands");
                }
            }
            finally
            {
...
public class SuppressBackspaceMessageAttribute : ICommandLineArgumentsDefinitionMetadata { }
