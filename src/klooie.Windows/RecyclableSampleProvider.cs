...
internal class RecyclableSampleProvider : Recyclable, ISampleProvider
{
    ...
    protected virtual void Construct(EventLoop eventLoop, CachedSound sound, float volume, ILifetime loopLifetime, bool loop)
    {
    ...
    public static RecyclableSampleProvider Create(EventLoop eventLoop, CachedSound sound, float volume, ILifetime? loopLifetime, bool loop)
    {
        var ret = pool.Value.Rent();
        ret.Construct(eventLoop, sound, volume, loopLifetime, loop);
        return ret;
    }
    ...
    public int Read(float[] buffer, int offset, int count)
    {
        ...
        while (count > 0)
        {
            ...
            else
            {
                ...
                {
                    buffer[offset + i] = sound.AudioData[intPosition + i] * Volume;
                }
            }
